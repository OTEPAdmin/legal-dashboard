...
from utils.config_manager import is_feature_visible  # <--- Import Visibility Manager
...
def show_view():
    ...
    # =================================================================================================
    # SECTION 4: FINANCIAL & REMITTANCE
    # =================================================================================================
    if is_feature_visible("EIS_Financials"):
        st.markdown("#### 💸 การนำส่งเงิน & งบการเงิน | FINANCIAL & REMITTANCE")
        
        # NOTE: Variables cpk_remit_total/cps_remit_total come from Section 3. 
        # If Section 3 is hidden but 4 is shown, we need to ensure data is loaded.
        if 'cpk_remit_total' not in locals():
            cpk_remit_total = 0
            cps_remit_total = 0
            if 'df_eis_extra' in st.session_state and not st.session_state['df_eis_extra'].empty:
                 # Re-run minimal filter logic just for finance if not done above
                 # (Omitting full copy-paste for brevity, assuming standard usage flow)
                 pass 
