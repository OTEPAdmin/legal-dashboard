...
def show_view():
    ...
    def get_val(cat, item):
        ...
        subset = df_snap[(df_snap['Category'] == cat) & (df_snap['Item'] == item)]
    ...
    with col1:
        ...
        if not df_death.empty:
            ...
            fig = px.bar(df_death_agg, x='Value', y='Item', orientation='h', text='Value', color='Item')
            fig.update_layout(showlegend=False, height=350, xaxis_title="จำนวน", yaxis_title=None)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("ไม่พบข้อมูลสาเหตุการเสียชีวิต")

    with col2:
        st.subheader("💰 งบการเงิน")
        df_fin = df_filtered[df_filtered['Category'] == 'Financial']
        if not df_fin.empty:
            fig = px.bar(df_fin, x='Month', y='Value', color='Item', barmode='group',
                             color_discrete_map={'รายรับ': '#4CAF50', 'รายจ่าย': '#F44336'})
            fig.update_layout(height=350, xaxis_title=None, yaxis_title="ล้านบาท")
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("ไม่พบข้อมูลเกี่ยวงบการเงิน")
    ...
    if not df_remit.empty:
        ...
        fig = px.area(df_remit, x='Month', y='Value', color='Item', 
                            color_discrete_map={'เงินนำส่ง ช.พ.ค.': '#2196F3', 'เงินนำส่ง ช.พ.ส.': '#9C27B0'})
        fig.update_layout(height=400, xaxis_title="เดือน", yaxis_title="จำนวนเงิน (ล้านบาท)")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("ไม่พบข้อมูลการนำส่งเงิน")
