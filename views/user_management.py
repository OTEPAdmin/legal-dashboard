...
def show_view():
    ...
    DASHBOARD_OPTIONS = [
        "สำนัก ช.พ.ค. - ช.พ.ส",
        "สำนักการคลัง - กลุ่มการเงิน",
        "สำนักการคลัง - กลุ่มการพัสดุและอาคารสถานที่",
        "สำนักการคลัง - กลุ่มบัญชี",
    ...
    with tab1:
        ...
        with c1: user_to_del = st.selectbox("เลือกผู้ใช้ที่ต้องการลบ", list(users.keys()))
        with c2:
            st.write("") 
            st.write("") 
        ...
        if edit_user_key:
            ...
            with st.form("edit_user_form"):
                ...
                if e_role == "User":
                    ...
                    # Auto-Map old names if present (Optional but helpful)
                    valid_defaults = []
                    for v in current_views:
                        if v in DASHBOARD_OPTIONS: valid_defaults.append(v)
    ...
    with tab4:
        ...
        col1, col2 = st.columns(2)
        with col1: target_user = st.selectbox("เลือกผู้ใช้", list(users.keys()), key="pwd_user_select")
        with col2: new_pwd = st.text_input("รหัสผ่านใหม่", type="password", key="pwd_new")
        ...
        if st.button("อัปเดตรหัสผ่าน"):
            ...
            else: st.warning("⚠️ กรุณากรอกรหัสผ่านใหม่")
